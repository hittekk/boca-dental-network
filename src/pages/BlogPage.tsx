              ...
              {posts.map((p) => (
                <Link key={p.id} to={`/blog/${p.slug}/`} style={{ textDecoration: 'none', color: DARK, background: 'white', border: '1px solid rgba(0,29,61,0.08)', borderRadius: 16, overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 6px 18px rgba(0,29,61,0.04)' }}>
                  ...
                  <div style={{ padding: '20px 22px 24px', display: 'flex', flexDirection: 'column', gap: 8, flex: 1 }}>
                    {p.published_at && <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, color: 'rgba(0,29,61,0.45)' }}>{fmtDate(p.published_at)}</div>}
...
export function BlogPostPage() {
  ...
  const origin = typeof window !== 'undefined' ? window.location.origin : ''
  const schema = usePageMeta({
    title: post ? `${post.title} | Boca Dental and Braces` : 'Blog | Boca Dental and Braces',
    description: post?.excerpt || 'Boca Dental and Braces blog.',
      ...
      <section style={{ background: 'linear-gradient(180deg, #F7F9FC 0%, white 70%)', padding: '160px 32px 32px' }}>
        <div style={{ maxWidth: 820, margin: '0 auto' }}>
          ...
          {loading ? (
            ...
            <>
              {post.published_at && <div style={{ fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1.2, color: ORANGE, marginBottom: 10 }}>{fmtDate(post.published_at)}{post.author ? ` · ${post.author}` : ''}</div>}
